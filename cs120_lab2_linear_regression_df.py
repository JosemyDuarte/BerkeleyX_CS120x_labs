# Databricks notebook source exported at Mon, 25 Jul 2016 01:39:29 UTC
# MAGIC %md
...
def parse_points(df):
    ...
    """
    return df.select(sql_functions.split(df.value, ",").alias("values")).map(lambda f: LabeledPoint(f.values[0],f.values[1:])).toDF()
...
content_stats = (parsed_points_df
                 . selectExpr("max(label)","min(label)"))

min_year = content_stats.select("min(label)").first()[0]
max_year = content_stats.select("max(label)").first()[0]
...
# TODO: Replace <FILL IN> with appropriate code
preds_and_labels_train = parsed_train_data_df.map(lambda t: (average_train_year,t.label))
...
preds_and_labels_val = parsed_val_data_df.map(lambda t: (average_train_year,t.label))
...
preds_and_labels_test = parsed_test_data_df.map(lambda t: (average_train_year,t.label))
...
def linreg_gradient_descent(train_data, num_iters):
    ...
    for i in range(num_iters):
        ...
        preds_and_labels_train = train_data.map(lambda t: get_labeled_prediction(w,t))
        preds_and_labels_train_df = sqlContext.createDataFrame(preds_and_labels_train, ["prediction", "label"])
        ...
        gradient = train_data.map(lambda t: gradient_summand(w,t)).sum()
...
preds_and_labels = (parsed_val_data_df
                      .map(lambda t: get_labeled_prediction(weights_LR0,t)))
preds_and_labels_df = sqlContext.createDataFrame(preds_and_labels, ["prediction", "label"])
...
def two_way_interactions(lp):
    ...
    """
    my_features = DenseVector([i[0] * i[1] for i in itertools.product(lp.features, repeat=2)])
    two_way_interactions = np.hstack((lp.features, my_features))
    return LabeledPoint(lp.label, two_way_interactions)
