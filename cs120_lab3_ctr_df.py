# Databricks notebook source exported at Wed, 3 Aug 2016 03:32:11 UTC
# MAGIC %md
...
sample_distinct_feats_df = (sample_data_df
                              .select(explode(sample_data_df.features)).distinct())
...
def create_one_hot_dict(input_df):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return input_df.select(explode(input_df.features)).distinct().map(lambda t: tuple(t[0])).zipWithIndex().collectAsMap()
...
def parse_point(point):
    ...
    return [(idx-1,feature) for idx, feature in enumerate(point.split(",")) if idx > 0]
...
ohe_validation_df = (parse_raw_df(raw_validation_df).select(ohe_dict_missing_udf(col("feature")).alias("features"),col("label"))).cache()
...
def add_log_loss(df):
    ...
    """
    return (df.select(when(df.p == 0, df.p+epsilon).when(df.p == 1, df.p-epsilon).otherwise(df.p).alias("p"), df.label)
            .withColumn("log_loss",when(col("label") == 1, -log(col("p"))).otherwise(-log(1-col("p")))))
...
class_one_frac_train = (ohe_train_df.agg({"label": "mean"}).first()[0])
print 'Training class one fraction = {0:.3f}'.format(class_one_frac_train)
log_loss_tr_base = add_log_loss(ohe_train_df.withColumn("p", lit(class_one_frac_train))).agg({"log_loss": "mean"}).first()[0]
...
def add_probability(df, model):
    ...
    def get_p(features):
        ...
        rawPrediction =  features.dot(coefficients_broadcast.value) + intercept
        # Bound the raw value between 20 and -20
        rawPrediction = max(min(rawPrediction, 20), -20)
...
def evaluate_results(df, model, baseline=None):
    ...
    log_loss = with_log_loss_df.agg({"log_loss": "mean"}).first()[0]
    return log_loss
...
# TODO: Replace <FILL IN> with appropriate code
log_loss_val_base = add_log_loss(ohe_validation_df.withColumn("p", lit(class_one_frac_train))).agg({"log_loss": "mean"}).first()[0]
...
def get_sparsity(df):
    ...
    """
    return df.select(feature_sparsity_udf(col("features")).alias("sparse")).agg({"sparse": "avg"}).first()[0]
...
class_one_frac_test = (hash_test_df.agg({"label": "mean"}).first()[0])

print 'Class one fraction for test data: {0}'.format(class_one_frac_test)
log_loss_test_baseline = add_log_loss(hash_test_df.withColumn("p", lit(class_one_frac_test))).agg({"log_loss": "mean"}).first()[0]
